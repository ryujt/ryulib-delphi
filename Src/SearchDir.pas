...
procedure SearchFiles(APath:string; ARecursive:boolean; AProcedure:TIterateProcedure); overload;
procedure SearchFiles(APath,AFilename:string; ARecursive:boolean; AProcedure:TIterateProcedure); overload;
...
procedure SearchFiles(APath,AFilename:string; ARecursive:boolean; AProcedure:TIterateProcedure);
var
  SearchRec : TSearchRec;
  iSearchResult : integer;
  isDirectory : boolean;
...
begin
  ...
  iSearchResult := FindFirst(APath + AFilename, faAnyFile, SearchRec);
  while iSearchResult = 0 do begin
    // 디랙토리 중 무시해야할 것들
    if (SearchRec.Name = '.' ) or (SearchRec.Name = '..') then begin
      iSearchResult := FindNext(SearchRec);
